...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['save_details'])) {
        // Update team_section section data
        $title = $_POST['title'] ?? '';
        ...
        // Update database
        $data = ['team_section' => $json_encoded_data];
        $result = $crud->update('about_us', $data, ['id' => 1]);

        if ($result === true) {
            $_SESSION['status'] = "Team section updated successfully";
        } else {
            $_SESSION['status_error'] = "Team section update failed";
        }

        header('Location: ' . $GLOBALS['site_url'] . 'about/team_section');
        exit();
    }
}

// Fetch team_section section data
$section_title = $our_team_section['title'] ?? '';
$section_heading = $our_team_section['heading'] ?? '';
$section_description = $our_team_section['description'] ?? '';
?>
    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper" style="min-height: 107px;">
            <div class="card card-primary card-body">
                ...
                <div class="card-header">Edit Our Team Section</div>
                <form action="<?php echo htmlspecialchars($GLOBALS['site_url']); ?>about/team_section" method="post">
                    <div class="card card-primary card-body">
