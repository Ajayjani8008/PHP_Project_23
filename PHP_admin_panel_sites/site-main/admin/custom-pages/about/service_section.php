...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['save_details'])) {
        // Update service_section section data
        $title = $_POST['title'] ?? '';
        ...
        // Update database
        $data = ['service_section' => $json_encoded_data];
        $result = $crud->update('about_us', $data, ['id' => 1]);

        if ($result === true) {
            $_SESSION['status'] = "Service section updated successfully";
        } else {
            $_SESSION['status_error'] = "Service section update failed";
        }

        header('Location: ' . $GLOBALS['site_url'] . 'about/service_section');
        exit();
    }
}

// Fetch service_section section data
$section_title = $service_data['title'] ?? '';
$section_heading = $service_data['heading'] ?? '';
$section_description = $service_data['description'] ?? '';
?>
    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper" style="min-height: 107px;">
            <div class="card card-primary card-body">
                ...
                <form action="<?php echo htmlspecialchars($GLOBALS['site_url']); ?>about/service_section" method="post">
                    <div class="card card-primary card-body">
